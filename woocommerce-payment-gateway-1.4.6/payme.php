...
add_action( 'woocommerce_product_options_pricing', 'payme_ikpu_field' );
function payme_ikpu_field() {
    woocommerce_wp_text_input( array(
        'id'                => 'payme_ikpu',
        'label'             => __( 'ИКПУ', 'woocommerce' ),
        'placeholder'       => 'ИКПУ',
        'desc_tip'          => 'true',
        'description'       => __( 'Идентификационный код продукции и услуг', 'woocommerce' ),
     ) );
}

add_action( 'woocommerce_product_options_pricing', 'payme_vat_field' );
function payme_vat_field() {
    woocommerce_wp_select(  array( 
        'id'          => 'payme_vat',
        'label'       => __( 'НДС%', 'woocommerce' ),
        'options'     => array(
            // ''        => __( 'Выберите процент НДС', 'woocommerce' ),
            '0'    => '0',
            '12' => '12',
        )
    ) );
...
add_action( 'woocommerce_product_options_pricing', 'payme_package_code_field' );
function payme_package_code_field() {
    woocommerce_wp_text_input( array(
        'id'                => 'payme_package_code',
        'label'             => __( 'Код упаковки или услуги', 'woocommerce' ),
        'placeholder'       => '',
        'desc_tip'          => 'true',
        'description'       => __( 'Единица измерения', 'woocommerce' ),
     ) );
}


function payme_fiscal_save( $post_id ) {

	$woocommerce_IKPU = $_POST['payme_ikpu'];
	if (  isset( $woocommerce_IKPU ) ) {
		update_post_meta( $post_id, 'payme_ikpu', esc_attr( $woocommerce_IKPU ) );
	}

    $woocommerce_vat = $_POST['payme_vat'];
	if (  isset( $woocommerce_vat ) ) {
		update_post_meta( $post_id, 'payme_vat', esc_attr( $woocommerce_vat ) );
	}

    $woocommerce_package_code = $_POST['payme_package_code'];
	if (  isset( $woocommerce_package_code ) ) {
		update_post_meta( $post_id, 'payme_package_code', esc_attr( $woocommerce_package_code ) );
	}
}

add_action( 'woocommerce_process_product_meta', 'payme_fiscal_save', 10 );
...
function woocommerce_payme()
{
    ...
    class WC_PAYME extends WC_Payment_Gateway
    {
        ...
        public function __construct()
        {
            ...
            $this->fiscal = $this->get_option('fiscal');
        ...
        public function generate_form($order_id)
        {
            ...
            foreach ( WC()->cart->get_cart() as $cart_item ) {
                ...
                $id = $product->get_id();
                if(!empty($product)){
                    $order->update_meta_data( "Payme_ikpu[{$id}]", $product->get_meta('payme_ikpu') );
                    $order->update_meta_data( "Payme_vat[{$id}]", $product->get_meta('payme_vat') );
                    $order->update_meta_data( "Payme_package_code[{$id}]", $product->get_meta('payme_package_code') );
                    $order->update_meta_data( "Payme_price[{$id}]", $product->get_regular_price() );
                    $order->save();
                }
            }
...
FORM;
        ...
        private function get_order_by_date($from, $to)
        {
            global $wpdb;

            try {
                // $prepared_sql = $wpdb->prepare("SELECT post_id FROM $wpdb->postmeta WHERE meta_value = '%s' AND meta_key = '_payme_create_time'", $date);
                $prepared_sql = $wpdb->prepare("SELECT post_id FROM $wpdb->postmeta 
                WHERE meta_value  BETWEEN '%d' AND '%s' AND meta_key = '_payme_create_time'", $from, $to);
                $order_id[] = $wpdb->get_results($prepared_sql);
        ...
        private function CheckPerformTransaction($payload)
        {
            ...
            if ($amount != $payload['params']['amount']) {
                ...
                foreach ( $items as $item ) {
                    ...
                    $ikpu = get_post_meta( $order->get_id(), "Payme_ikpu[{$product_id}]", true );
                    $count = $item->get_quantity();
                    $total_nodiscount = get_post_meta( $order->get_id(), "Payme_price[{$product_id}]", true );;     
                    $total  = $item->get_total();
                    $total_discount = $total / $count;
                    $discount =  $this->amount_to_coin($total_nodiscount - $total_discount);
                    $total_discount = ($discount == 0) ? null : $discount;
                    $price = $this->amount_to_coin($total_nodiscount);
                    $vat_percent = get_post_meta( $order->get_id(), "Payme_vat[{$product_id}]", true );
                    $package_code = get_post_meta( $order->get_id(), "Payme_package_code[{$product_id}]", true );
